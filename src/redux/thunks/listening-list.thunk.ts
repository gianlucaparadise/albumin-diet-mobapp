...
import {
  loadListeningListAction,
  loadNextListeningListAction,
  errorListeningListAction,
} from '../actions/listening-list.actions';
import { UserAlbum } from 'albumin-diet-types';
import { updateMyAlbum } from './my-albums.thunk';
import { updateSearchAlbum } from './search.thunk';

export const loadListeningList = (): ThunkAction<
  void,
  AppState,
  null,
  AnyAction
> => async (dispatch) => {
  try {
    ...
    dispatch(loadListeningListAction(response.data));
  } catch (error) {
    console.log('Error while loadListeningList');
    console.log(error);
    dispatch(errorListeningListAction(error));
  }
};

export const loadListeningListNext = (): ThunkAction<
  void,
  AppState,
  null,
  AnyAction
> => async (dispatch, getState) => {
  try {
    ...
    dispatch(loadNextListeningListAction(response.data));
  } catch (error) {
...
export const addToLocalListeningList = (
  ...
  try {
    ...
    dispatch(loadListeningListAction(listeningList));
...
export const removeFromLocalListeningList = (
  albumId: string,
): ThunkAction<void, AppState, null, AnyAction> => async (
  dispatch,
  getState,
  ...
  try {
    ...
    listeningList.splice(toBeRemoved, 1);

    dispatch(loadListeningListAction(listeningList));

    // Refresh MyAlbums
    dispatch(updateMyAlbum(albumId, { isInListeningList: false }));

    // Refresh SearchAlbums
    dispatch(updateSearchAlbum(albumId, { isInListeningList: false }));
  } catch (error) {
