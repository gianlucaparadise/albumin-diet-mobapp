...
import {
  AlbumEggableMap,
  getCanBeEgged,
} from '../redux/reducers/album-detail.reducer';
...
class AlbumCardWidget extends Component<Props, State> {
  ...
  onLayout = (event: LayoutChangeEvent) => {
    ...
    if (this.contentView && this.props.scrollView) {
      ...
      if (parentNode) {
        this.contentView.measureLayout(parentNode, this.onMeasured, () => {
  ...
  onMeasured = (left: number, top: number) => {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View onStartShouldSetResponder={() => true}>
              <Chip textStyle={styles.listItemText} style={[styles.listItem]}>
                {item.name}
              </Chip>
            </View>
          )}
      ...
      <View
        ...
        <Card onPress={this.onPressed} style={this.props.style} elevation={3}>
          ...
          <Animated.View
            style={[
              styles.content,
              this.opacityTransform(this.state.yScreenOffset),
            ]}>
            <Text numberOfLines={1} ellipsizeMode={'tail'} style={styles.small}>
              {this.artistName}
            </Text>
            <Text numberOfLines={1} ellipsizeMode={'tail'} style={styles.small}>
              {this.releaseYear}
            </Text>
            <Text numberOfLines={2} ellipsizeMode={'tail'} style={styles.big}>
              {this.albumName}
            </Text>
            <Card.Actions>
              <ToggleIconButton
                ...
                enabled={getCanBeEgged(
                  this.props.canAlbumBeEggedMap,
