...
export const loadSearch = (
  ...
  try {
    ...
    dispatch(loadSearchAction(query, response.data));
  } catch (error) {
    console.log('Error while loadSearch');
    console.log(error);
    dispatch(errorSearchAction(error));
  }
};

export const loadSearchNext = (): ThunkAction<
  void,
  AppState,
  null,
  AnyAction
> => async (dispatch, getState) => {
  try {
    ...
    const response = await ConnectionHelper.Instance.searchAlbums(
      query,
      offset,
    );
    dispatch(loadNextSearchAction(response.data));
  } catch (error) {
 ...
 */
export const updateSearchAlbum = (
  albumId: string,
  props: any,
  ...
  try {
    ...
    dispatch(loadSearchAction(state.query || '', albums));
  } catch (error) {
