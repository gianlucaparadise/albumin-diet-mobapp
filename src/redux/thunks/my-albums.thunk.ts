...
import {
  loadMyAlbumsAction,
  loadNextMyAlbumsAction,
  errorMyAlbumsAction,
} from '../actions/my-albums.actions';
import { TaggedAlbum } from 'albumin-diet-types';

export const loadMyAlbums = (
  tags: string[],
  showUntagged: boolean,
): ThunkAction<void, AppState, null, AnyAction> => async (dispatch) => {
  try {
    const response = await ConnectionHelper.Instance.getAlbums(
      tags,
      showUntagged,
    );
    dispatch(loadMyAlbumsAction(tags, showUntagged, response.data));
  } catch (error) {
    console.log('Error while loadMyAlbums');
    console.log(error);
    dispatch(errorMyAlbumsAction(error));
  }
};

export const loadMyAlbumsNext = (): ThunkAction<
  void,
  AppState,
  null,
  AnyAction
> => async (dispatch, getState) => {
  try {
    ...
    dispatch(loadNextMyAlbumsAction(response.data));
  } catch (error) {
...
export const addToMyAlbums = (
  ...
  dispatch(loadMyAlbums(tags, showUntagged));
};

export const removeFromMyAlbums = (
  albumId: string,
): ThunkAction<void, AppState, null, AnyAction> => async (
  dispatch,
  getState,
  ...
  try {
    ...
    myAlbums.splice(toBeRemoved, 1);

    dispatch(loadMyAlbumsAction(tags, showUntagged, myAlbums));
  } catch (error) {
...
export const updateMyAlbum = (
  ...
  try {
    ...
    dispatch(loadMyAlbumsAction(state.tags, state.showUntagged, albums));
  } catch (error) {
