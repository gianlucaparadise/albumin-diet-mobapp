...
import { StyleSheet, View, Image, Animated, Easing, StyleProp, ImageStyle, ViewStyle } from 'react-native';
...
export default class SplashScreen extends Component<Props> {
	...
	animate() {
		...
		const fadeTiming = Animated.timing(
		...
		const rotationTiming = Animated.timing(
			this.headphonesAnimatedValue,
			{
				toValue: 1,
				duration: 300,
				easing: Easing.exp,
		...
		Animated.sequence([fadeTiming, rotationTiming]).start(() => this.waitAndGo());
	}
		...
		return (
			<View style={styles.container} onStartShouldSetResponder={() => this.onPressGo()}>
				<View style={styles.logoContainer}>
					<Animated.Image resizeMode="contain" style={[styles.logo, eggStyle]} source={require('./../../resources/images/albumin-diet-egg.png')} ></Animated.Image>
					<Animated.Image resizeMode="contain" style={[styles.logo, headphonesStyle]} source={require('./../../resources/images/albumin-diet-headphones.png')} ></Animated.Image>
				</View>
				<Animated.Text style={[styles.welcome, eggStyle]}>Albumin Diet</Animated.Text>
			</View>
		);
	}
}
